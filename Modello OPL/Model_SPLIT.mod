 ...
 *********************************************/
 ...
 // Multicommodity flow problem e network design problem con traffici SPLITTABLE
 ...
 dvar boolean x[INs]; // variabile binaria che indica se un TP è assegnato a uno specifico CS
 dvar boolean z[CSs]; // variabile binaria che indica se un router è installato presso uno specifico CS
 dvar boolean w[OUTs]; // variabile binaria che indica se un CS è connesso a uno specifico DN
 ...
 subject to{
   ...
   forall(i in TPs, k in DNs: Traffico[i][k] != 0){
     forall(j in CSs){
       vinConsTraffico:
